...
class AIAgent:
    ...
    def __init__(
        ...
        # Scheduling config: context threshold for local model routing
        self._context_threshold_tokens = 0  # 0 = disabled
        try:
            _scheduling_cfg = _agent_cfg.get("scheduling", {})
            if isinstance(_scheduling_cfg, dict):
                self._context_threshold_tokens = int(
                    _scheduling_cfg.get("context_threshold_tokens", 0)
                )
        except Exception:
            pass
        ...
        # for reliable tool-calling workflows (64K tokens).  Skip this check
        # when the user explicitly set model.context_length in config.yaml.
        _config_ctx = getattr(self, "_config_context_length", None)
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        # deepseek-r1 via Ollama does not support tool calling — strip tools
        # so the model responds with plain text; tool-requiring tasks fall
        # back through fallback_providers chain to DeepSeek API.
        if tools_for_api and self.base_url and "ollama" in self.base_url.lower():
            _model_lower = (self.model or "").lower()
            if "deepseek-r1" in _model_lower:
                tools_for_api = None
    ...
    def run_conversation(
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # Trim old messages so the local model always sees a
            # manageable context window.  This prevents unbounded
            # history accumulation from forcing every turn to cloud.
            if (
                self._context_threshold_tokens > 0
                and not self._fallback_activated
            ):
                _budget = self._context_threshold_tokens
                _tokens_per_msg = [estimate_messages_tokens_rough([m]) for m in api_messages]
                _total = sum(_tokens_per_msg)
                if _total > _budget:
                    # Walk backwards, keep as many as fit in budget
                    _running = 0
                    _cut = len(api_messages)  # first index to keep
                    for i in range(len(api_messages) - 1, -1, -1):
                        if _running + _tokens_per_msg[i] > _budget and _running > 0:
                            _cut = i + 1
                            break
                        _running += _tokens_per_msg[i]
                    _dropped = _cut
                    api_messages = api_messages[_cut:]
                    approx_tokens = _running
                    if _dropped > 0:
                        self._vprint(
                            f"{self.log_prefix}   📊 Rolling window: dropped {_dropped} old "
                            f"message(s), ~{approx_tokens:,} tokens (budget {_budget:,})",
                            force=True,
                        )

                # If even the most recent messages exceed budget,
                # route to fallback (message itself is too large).
                if approx_tokens > _budget and self._fallback_chain:
                    self._vprint(
                        f"{self.log_prefix}   📊 Context ~{approx_tokens:,} tokens exceeds "
                        f"budget {_budget:,} even after trimming — routing to fallback...",
                        force=True,
                    )
